...
import banduty.knightsheraldry.items.armor.attachment.KHAragoneseSalletHelmet;
import banduty.knightsheraldry.model.AragoneseSalletClosed;
import banduty.knightsheraldry.model.AragoneseSalletOpened;
...
public class KHAragoneseSalletHelmetAttachmentRenderer implements ArmorAttachmentRenderer {
    @Override
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, LivingEntity livingEntity,
        ...
        if (!(itemStack.getItem() instanceof KHAragoneseSalletHelmet khAragoneseSalletHelmet)) return;
        if (khAragoneseSalletHelmet.hasOpenVisor(itemStack)) {
            boolean current = itemStack.getOrDefault(SCDataComponents.VISOR_OPEN.get(), false);
            if (current) model = new AragoneseSalletOpened(AragoneseSalletOpened.getTexturedModelData().bakeRoot());
            else model = new AragoneseSalletClosed(AragoneseSalletClosed.getTexturedModelData().bakeRoot());
        } else model = new AragoneseSalletClosed(AragoneseSalletClosed.getTexturedModelData().bakeRoot());
