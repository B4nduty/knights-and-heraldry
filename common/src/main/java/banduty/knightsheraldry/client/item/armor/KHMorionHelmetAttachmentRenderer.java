...
import banduty.knightsheraldry.items.armor.attachment.KHMorionHelmet;
import banduty.knightsheraldry.model.MorionClosedModel;
import banduty.knightsheraldry.model.MorionOpenedModel;
...
public class KHMorionHelmetAttachmentRenderer implements ArmorAttachmentRenderer {
    @Override
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, LivingEntity livingEntity,
        ...
        if (!(itemStack.getItem() instanceof KHMorionHelmet khMorionHelmet)) return;
        if (khMorionHelmet.hasOpenVisor(itemStack)) {
            boolean current = itemStack.getOrDefault(SCDataComponents.VISOR_OPEN.get(), false);
            if (current) model = new MorionOpenedModel(MorionOpenedModel.getTexturedModelData().bakeRoot());
            else model = new MorionClosedModel(MorionClosedModel.getTexturedModelData().bakeRoot());
        } else model = new MorionClosedModel(MorionClosedModel.getTexturedModelData().bakeRoot());
